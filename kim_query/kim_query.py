...
def get_test_result(test, model, prop, keys, units):
    ...
    Returns
    -------
    A nested list, with the outer list looping over the different instances
    of 'prop' computed by the 'test'. In the majority of cases, the outer
    list will only have one element. The inner list loops over the values of 
    each key listed in 'keys' in the units specified by 'units'.
    If no matching results are found, an empty list is returned.
    """
